...
pub fn generate(missile: &Missile, launch_parameters: LaunchParameter, timestep: f64, debug: bool) -> LaunchResults {
	...
	//Equiv airspeed
	let mut eas = launch_parameters.start_velocity;
	...
	for i in 0..sim_len {
		drag_force = Force::from_N(0.5 * rho * eas.to_mps().powi(2) * missile.cxk * area);
		...
		eas += Velocity::from_mps(a.to_mps2()) * timestep;
		let tas = ias_to_tas(eas, &atmosphere, altitude);
		...
		if debug {
			println!("ts(s): {} D(m): {} Dt(m): {} a(m/s²): {} v(m/s): {} d(N): {} rho: {} m(kg): {} s: {}",
					 ...
					 eas.to_mps().round().to_string().pad_to_width(4),
